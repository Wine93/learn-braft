...
在[<3.2 选举优化>](/ch03/3.2/optimization.md)中提到过，这种场景 `PreVote` 无法阻止，但是可以利用 `Check Quorum` 解决。但是其实在目前 braft 的实现中，这种场景是不会出现的，因为 Leader 是将新配置（即 `C{new}`）复制到新集群的大多数集群后，才开始移除不在新集群中的 `Replicator`（停止广播心跳），而不是在将新配置未达到 `Quorum` 前停止心跳。所以对于节点 `S1` 来说，
